        ...
        <ul className="grid list-none grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {siteOfferings.map((item) => (
            <li key={item.key} className="min-w-0">
              <ToolCard>
                <h3 className="mb-3 text-center text-xl font-semibold text-ink md:text-2xl">
                  {item.title}
                </h3>
                <p className="mb-6 flex-1 leading-7 text-ink-muted">
                  {item.description}
                </p>

                {item.kind === "internal" && (
                  <div className="mt-auto flex justify-center border-t border-line pt-5">
                ...
                {item.kind === "placeholder" && (
                  <p className="mt-auto border-t border-line pt-5 text-center text-sm text-ink-faint">
                    敬請期待
                  </p>
                )}
              </ToolCard>
